    ...
    <div style={{ position:'absolute', inset:0, zIndex:60, display:'flex', flexDirection:'column', animation:'sheet-fade 0.3s ease-out' }}>
      ...
      <div style={{ position:'relative', zIndex:1, flex:1, display:'flex', flexDirection:'column', padding:'14px 14px 0', minHeight:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, flexShrink:0 }}>
          ...
          <button onClick={onSkip} style={{
            ...
            color:'#94a3b8', borderRadius:20, padding:'7px 16px', cursor:'pointer',
            ...
            {teamA.map(pid => {
              ...
              return (
                <div key={pid} style={{ flex:1, background:'rgba(74,222,128,0.04)', border:'1px solid rgba(74,222,128,0.15)', borderRadius:12, padding:'10px 8px', display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
                  {/* Avatar — 2pt bigger: 46px */}
                  <div style={{ width:46, height:46, borderRadius:'50%', overflow:'hidden', border:`1.5px solid ${level.aura}`, background:'#1a2a1a', flexShrink:0 }}>
                    <img src={getAvatarUrl(pid)} width={46} height={46} style={{width:'100%',height:'100%',objectFit:'cover'}} onError={e=>e.target.style.display='none'}/>
                  </div>
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:'#4ade80', letterSpacing:0.5, lineHeight:1, textAlign:'center' }}>{p.display_name}</div>
            ...
            {teamB.map(pid => {
              ...
              return (
                <div key={pid} style={{ flex:1, background:'rgba(96,165,250,0.04)', border:'1px solid rgba(96,165,250,0.15)', borderRadius:12, padding:'10px 8px', display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
                  ...
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:13, color:'#60a5fa', letterSpacing:0.5, lineHeight:1, textAlign:'center' }}>{p.display_name}</div>
                  <div style={{ height:1, background:'rgba(255,255,255,0.05)', width:'100%' }}/>
                  <div style={{ display:'flex', flexWrap:'wrap', gap:4, justifyContent:'center' }}>
                    {SKILLS_DEF.map(s => {
                      const on = pSel.has(s.key)
      ...
      <div style={{ position:'relative', zIndex:1, padding:'10px 14px 24px', flexShrink:0 }}>
        <button onClick={()=>onSubmit(sel)} style={{
          width:'100%',
          background: anySelected?'linear-gradient(135deg,#14532d,#166534)':'rgba(255,255,255,0.05)',
          border:`1.5px solid ${anySelected?'#4ade80':'rgba(255,255,255,0.12)'}`,
          color: anySelected?'#4ade80':'#64748b',
...
export default function LogGame({ onClose, onGameLogged }) {
  ...
  useEffect(() => {
    getPlayers().then(r => {
      if (r.success) {
        // Sort by total_games desc so most active players first
        const sorted = [...r.data].sort((a,b) => (b.total_games||0) - (a.total_games||0))
        setPlayers(sorted)
      }
    })
  }, [])
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:50, fontFamily:"'Rajdhani',sans-serif" }}>
      <style>{`
        ...
        .pc{border-radius:12px;padding:7px 3px;cursor:pointer;transition:all 0.2s;text-align:center;border:1px solid rgba(255,255,255,0.1);background:rgba(0,0,0,0.45);}
        ...
        .tt{flex:1;padding:11px;border-radius:12px;border:1.5px solid rgba(255,255,255,0.1);background:rgba(0,0,0,0.45);color:#475569;font-family:'Bebas Neue',sans-serif;font-size:15px;letter-spacing:2px;cursor:pointer;transition:all 0.2s;}
      ...
      {step===3&&winner&&(
        <div style={{position:'absolute',inset:0,zIndex:80,background:'rgba(0,0,0,0.9)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:20}}>
          ...
          <div style={{textAlign:'center',animation:'celebrate-in 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards',width:'100%',maxWidth:360}}>
            ...
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:64,color:'#ffd700',letterSpacing:5,lineHeight:1,textShadow:'0 0 40px rgba(255,215,0,0.8)',marginBottom:4}}>TEAM {winner}!</div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:42,color:'#4ade80',letterSpacing:8,marginBottom:20,animation:'score-pop 0.4s ease-out 0.4s both'}}>{countA} — {countB}</div>
      ...
      {step===1&&!showSkills&&(
        <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column'}}>
          ...
          <div style={{background:'rgba(6,13,20,0.98)',borderTop:'1px solid rgba(74,222,128,0.15)',borderRadius:'22px 22px 0 0',padding:'14px 14px 20px',animation:'panel-up 0.3s ease-out'}}>
            ...
            {players.length > 8 && (
              <div style={{fontSize:10,color:'#334155',textAlign:'center',marginBottom:8,fontFamily:"'Rajdhani',sans-serif"}}>↕ Scroll for more players</div>
            )}
